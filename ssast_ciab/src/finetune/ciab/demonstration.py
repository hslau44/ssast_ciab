...
def plot_attentions(attensions, fbank, nh, mean, std, batch_num=0, folder='.'):
    ...
    plt.show()
    attentions_filepath = os.path.join(folder,'attentions_0_pos.png')
    return attentions_filepath
...
def main_demo(model_path, folder, audio_file, method='patch', name='harry'):
    ...
    plt.close()
    result['fbank'] = os.path.join(folder,'fbank.png')
    fig, axs = plt.subplots(1,1, figsize=(8,20), sharex=True)
    axs.imshow(waveform)
    axs.axis('off')
    plt.savefig(os.path.join(folder,'waveform.png'), bbox_inches='tight')
    plt.close()
    result['waveform'] = os.path.join(folder,'waveform.png')
    ...
    attentions_filepath = plot_attentions(attentions, fbank, nh, args.dataset_mean, args.dataset_std, batch_num, folder=folder)
    result['attentions'] = attentions_filepath
    plt.close()
    ...
    plt.show()
    plt.close()
    result['prediction'] = attention_maps_filepath
